...
BOOST_AUTO_TEST_CASE(num_components_inherantence)
{
	...
	Manager<decltype(make_type_tuple<data1, data4>), decltype(make_type_tuple<decltype(base)>)>
		sister1;
	Manager<decltype(make_type_tuple<data4, data5>), decltype(make_type_tuple<decltype(base)>)>
		sister2;

	Manager<decltype(make_type_tuple<data6>),
			decltype(make_type_tuple<decltype(sister1), decltype(sister2)>)> child;
