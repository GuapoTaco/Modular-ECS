...
auto removeTypeAddPtr = [](auto arg) { return (typename decltype(arg)::type*){}; };
}
...
{
	static constexpr auto manager_type = boost::hana::type_c<manager<components_, bases_>>;
	/**
	 * @brief Gets the list of components owned by the manager.
	 *
	 * @return a boost::hana::tuple<> of components
	 */
	static constexpr auto my_components = boost::hana::make<components_>();
	/**
	 * @brief returns the direct bases of the manager.
	 *
	 * @return a boost::hana::tuple<> of components
	 */
	static constexpr auto my_bases = boost::hana::make<bases_>();
	/**
	 ...
	 */
	constexpr static auto all_components = [] {
		auto foldLam = [](auto arg) { return decltype(arg)::type::all_components; };
		return remove_dups(
	...
	static constexpr auto my_storage_components = [] {
		...
		return boost::hana::fold(my_components, boost::hana::make_tuple(), foldLam);
	}();
	...
	static constexpr auto all_storage_components = [] {
		...
		return boost::hana::fold(all_components, boost::hana::make_tuple(), foldLam);
	}();
	...
	{
		return boost::hana::if_(boost::hana::contains(all_components, component),
	...
	{
		return boost::hana::if_(isComponent(component),
	...
	{
		return boost::hana::if_(isStorageComponent(component),
	...
	{
		return boost::hana::if_(isStorageComponent(component),
								get_index_of_first_matching(my_storage_components, component),
								boost::hana::nothing);
	}
	...
	{
		return boost::hana::if_(boost::hana::contains(all_managers, manager),
	...
	{
		return boost::hana::if_(boost::hana::contains(my_bases, base),
								get_index_of_first_matching(my_bases, base), boost::hana::nothing);
	}
	 ...
	 *
	 * @return A boost::hana::type_c<...> of the managers that owns \c component. If \c component
	 * isn't a component, then it returns boost::hana::nothing
	 */
	template <typename T>
	static constexpr auto get_manager_from_component(T component)
	{
		...
		return boost::hana::if_(
			...
			boost::hana::fold(all_managers, boost::hana::type_c<boost::hana::none_t>, foldLam),
			boost::hana::nothing);
	}
	...
	static constexpr auto find_most_base_manager_for_signature(T signature)
	{
		...
		auto ret = boost::hana::while_(
			[](auto pair) { return pair[0_c] != pair[1_c]; },
	...
	static RuntimeSignature_t generate_runtime_signature(T signature)
	{
		...
		boost::hana::for_each(
			signature, [&ret](auto type) { ret[decltype(get_component_id(type))::value] = true; });
	...
	template <typename T>
	auto get_storage_component(T component, size_t handle) -> typename decltype(component)::type&
	{
		// TODO: implement
	}
	...
	{
		static_assert(manager_type == find_most_base_manager_for_signature(signature));
...
template <typename Types, typename Bases = decltype(boost::hana::make_tuple())>
auto create_manager(Types T, const Bases& bases = {})
{
	auto ismanagerptr = [](auto elem) {
		return boost::hana::traits::is_pointer(boost::hana::type_c<decltype(elem)>) &&
			   boost::hana::traits::is_base_of(boost::hana::type_c<manager_base>,
											   boost::hana::type_c<std::decay_t<decltype(*elem)>>);

	};
	// make sure all the bases are managers
	static_assert(decltype(boost::hana::all_of(bases,ismanagerptr))::value);

	// get bases types
	auto bases_noptr = boost::hana::transform(bases, [](auto elem) {
		return boost::hana::type_c<std::remove_pointer_t<decltype(elem)>>;
	});

	return manager<Types, decltype(bases_noptr)>(bases);
}
}
